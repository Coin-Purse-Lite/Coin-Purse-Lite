      ...
      <div className="news-list-scroll bg-white">
          {newsData.map((article, index) => (
              <div key={index} className="news-list-item bg-white">
                  <h1 className='bg-white' style={{fontWeight: '500'}}>{article.title}</h1>
                  <p className='bg-white'>{article.description}</p>
                  <a className='bg-white' style={{color: 'blue', textDecoration: 'underline'}} href={article.link} target="_blank" rel="noopener noreferrer">Read More</a>
              </div>
          ))}
      </div>
  </div>
